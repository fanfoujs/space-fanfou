...
这是一个基于原版太空饭否持续维护的 fork，可对外称为 **Space Fanfou 2.1**。
...
- 修复从网页直接复制图片后粘贴到输入框时，部分大图上传失败的问题
- 对浏览器剪贴板中体积异常膨胀的 PNG 图片做本地归一化，必要时转为更小的 JPEG 后再上传
- 优化图片上传失败时的错误提示，尽量展示饭否返回的真实报错文案，便于定位问题
